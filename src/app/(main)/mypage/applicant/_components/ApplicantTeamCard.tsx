      ...
      >
        <div className="flex w-full justify-between">
          ...
          <Tag className="bg-red-10 text-red-100">
            {application.recruitingPostStatus === '모집 마감'
              ? '마감'
              : formatDday(application.dday)}
          </Tag>
        </div>
        <div>
          <p className="body-5 flex items-start truncate text-gray-900">{application.title}</p>
          <p className="body-9 flex items-start text-gray-600">#{application.tags.join(' #')}</p>
        </div>
      </button>
      <button
        onClick={handleClick}
